...
const pluginName = 'maxmind_lite_asn';

if(isMainThread) {
    module.exports = (outputFile, download) => {
        return new Promise((resolve, reject) => {
            const worker = new Worker(__filename, {
                workerData: {outputFile, download},
        ...
        return new Promise(((resolve, reject) => {
            csvStream.on("data", function (r) {
                ...
                const output = stringify([[Math.min(ip2int(cidrInfo.firstHostAddress), ip2int(cidrInfo.lastHostAddress)),
                    Math.max(ip2int(cidrInfo.firstHostAddress), ip2int(cidrInfo.lastHostAddress)), meta]], {
                    delimiter: '|',
                    quote: '|'
                }); //
    ...
    const main = async (outputFile, download) => {
    ...
    main(workerData.outputFile, workerData.download)
